...
In this tutorial, we will cover the entire data science pipeline: data curation, parsing, and management; exploratory data analysis; hypothesis testing and machine learning. We will use R to carry out various techniques of predicting financial market movements, specifically stock prices of different cryptocurrencies.
...
We want to add the following new attributes for our data analysis later on: close ratio and spread.
...
cat("Opening Mean - ", mean(bitcoin$Open), "\n")
cat("Opening Median - ", median(bitcoin$Open), "\n")
cat("Opening Variance - ", var(bitcoin$Open), "\n")
cat("\n")
cat("Closing Mean - ", mean(bitcoin$Close), "\n")
cat("Closing Median - ", median(bitcoin$Close), "\n")
cat("Closing Variance - ", var(bitcoin$Close), "\n")
cat("\n")
cat("Spread Mean - ", mean(bitcoin$spread), "\n")
cat("Spread Median - ", median(bitcoin$spread), "\n")
cat("Spread Variance - ", var(bitcoin$spread), "\n")
cat("\n")
...
Now, we are going to plot the spread to see the best and worst days for each cryptocurrency. 
``` {r spread}
crypto_tab %>%
  group_by(coin_type) %>%
  arrange(desc(spread)) %>%
...
Next we want to analyze, the best days for Bitcoin, Litecoin and Ethereum. To do so, we will filter our data for the max spread for each coin type.
``` {r best day}
crypto_tab %>%
  group_by(coin_type) %>%
...
We see the best days for Bitcoin and Litecoin occur in December of 2017.

Let's also look at the minimum spread.

```{r worst day}
crypto_tab %>%
  group_by(coin_type) %>%
  filter(spread==min(spread))
```
Here we see Bitcoin and Ethereum both experience huge losses in January of 2018, signifying the 2018 cryptocurrency crash.
...
Looking at this plot, the close ratio seems to have a slight positive increasing linear trend over time. 
...
outcome_df <- crypto_tab %>%
   mutate(Direction = ifelse(spread>0, "up", "down"))
outcome_df
...
standardized_df <- predictor_df %>%
  group_by(coin_type) %>%
  mutate(mean = mean(spread)) %>%
  mutate(sd = sd(spread)) %>%
  mutate(z = (spread - mean) / sd) %>%
  ungroup()
standardized_df
