...
#Data Partition
samp_index <- sample(2, nrow(prediction_set), replace = T, prob = c(0.6,0.4))
trainer <- prediction_set[samp_index == 1,]
validator <- prediction_set[samp_index == 2,]

#Classifier
prediction_set$High <- as.factor(prediction_set$High)

#Formula
attributes <- names(prediction_set)
attributes <- attributes[!attributes %in% c("High")]
attributes1 <- paste(attributes, collapse = "+") #saves the column names separated by +
formula_rf <- as.formula(paste("High", attributes1, sep = " ~ "))
